...
<body>
        ...
        <div class="container">
            <div class="row d-flex align-items-center justify-content-center">
                ...
                <h1 class="text-white">
                    ...
                    <p class="text-white link-nav"><a href="staff-view-book-request.php"> Book Requests <span class="lnr lnr-arrow-right"></span></a></p>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="mx-auto table_container">
                ...
                <table id='books_issued' class="table table-bordered  ">
                    ...
                    <tbody>
                        ...
                        $result = PullData($con, 'book_requests', '*', '', '');
                        ...
                        ?>
                            <tr>
                                ...
                                <td><button type="button" class="btn" onclick="approve('<?php echo $row['reques_id'] ?>','<?php echo $row['request_by'] ?>')"><span class="badge badge-primary p-2"><i class="fa fa-plus-square mr-1" aria-hidden="true"></i>Approve</span></button>
                                <button type="button" onclick="cancel_book_request('<?php echo $row['reques_id'] ?>')" class="btn"><span class="badge badge-danger p-2"><i class="fa fa-times mr-1" aria-hidden="true"></i>Cancel</span></button></td>
                            </tr>
                        <?php
                        }
                        ?>
    ...
    <script>
        ...
        function send_approve_request(data) {
            var rq_id = data;
            hideModal('mymodal')
            $.ajax({
                type: "post",
        ...
        function approve(data,name) {

            var value = data;
            var html = "<p class='lead'>You are issuing this book to <b class='text-primary'>"+name+"</b> for next 15 days</p>"
            var footer = '<button type="button" class="btn btn-danger" data-dismiss="modal">Cancel</button><button type="button" class="btn btn-primary" onclick="send_approve_request(' + value + ')" >YES</button>'
            showModal('mymodal')
            $('#mymodal .icon-box').html('<i class="fa fa-plus-circle" aria-hidden="true"></i>');
            $('#mymodal .modal-title').html('Approve');
            $('#mymodal .modal-body').html(html);
            $('#mymodal .modal-footer').html(footer);
